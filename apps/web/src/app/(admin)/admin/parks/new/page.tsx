...
// ADMIN-044b — Create new Park form (ThaiPark DB Design)
...
export default function NewParkPage() {
  ...
  async function handleSave() {
    ...
    setIsSaving(true);
    // Mock save — replace with POST /api/admin/parks when Supabase is wired
  ...
  return (
    <div className="mx-auto max-w-[720px] px-4 py-6 sm:px-6">
      <div className="mb-5 flex items-center gap-2 text-sm text-muted">
        ...
        <span>/</span>
        <span className="text-ink">เพิ่มอุทยานใหม่</span>
      </div>

      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          <h1 className="font-display text-2xl font-bold text-forest-900">เพิ่มอุทยานใหม่</h1>
          <p className="mt-1 text-sm text-muted">กรอกข้อมูลอุทยานแห่งชาติตาม ThaiPark DB</p>
        </div>
      ...
      {saved && (
        <div className="mb-4 rounded-xl bg-success/10 px-4 py-3 text-sm font-medium text-success">
          ✓ บันทึกสำเร็จ กำลังกลับสู่รายการอุทยาน…
        </div>
      )}
      ...
      <div className="space-y-5">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label className="block text-xs font-medium text-forest-900">
                รหัสอุทยาน (park_code) <span className="text-danger">*</span>
              </label>
              <input
                type="text"
                value={form.parkCode}
                onChange={(e) => set('parkCode', e.target.value.toUpperCase())}
                placeholder="เช่น DNI"
                maxLength={10}
                className={inputCls('parkCode')}
              />
            ...
            <div>
              <label className="block text-xs font-medium text-forest-900">
                ชื่ออุทยาน (TH) <span className="text-danger">*</span>
              </label>
              <input
                type="text"
                value={form.nameTh}
                onChange={(e) => set('nameTh', e.target.value)}
                placeholder="เช่น อุทยานแห่งชาติดอยอินทนนท์"
                className={inputCls('nameTh')}
              />
            ...
            <div>
              ...
              <input
                ...
                placeholder="Doi Inthanon National Park"
            ...
            <div className="sm:col-span-2">
              ...
              <textarea
                ...
                placeholder="อธิบายอุทยานโดยย่อ"
        ...
        <section className="rounded-2xl border border-forest-800/10 bg-white p-5 shadow-sm">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="sm:col-span-2">
              <label className="block text-xs font-medium text-forest-900">URL รูปภาพหน้าปก (cover_image)</label>
              <input
                type="url"
                value={form.coverImage}
                onChange={(e) => set('coverImage', e.target.value)}
                placeholder="https://... หรือ /images/parks/..."
                className={inputCls('coverImage')}
              />
            </div>
            <div className="sm:col-span-2">
              <label className="block text-xs font-medium text-forest-900">URL แผนที่ Google Maps (map_url)</label>
              <input
                type="url"
                value={form.mapUrl}
        ...
        <section className="rounded-2xl border border-forest-800/10 bg-white p-5 shadow-sm">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label className="block text-xs font-medium text-forest-900">ละติจูด (latitude)</label>
              <input
                type="number"
                step="0.0001"
                value={form.latitude}
                onChange={(e) => set('latitude', e.target.value)}
                placeholder="เช่น 18.5887"
                className={inputCls('latitude')}
              />
              {errors.latitude && <p className="mt-1 text-xs text-danger">{errors.latitude}</p>}
            </div>
            <div>
              <label className="block text-xs font-medium text-forest-900">ลองจิจูด (longitude)</label>
              <input
                type="number"
                step="0.0001"
                value={form.longitude}
                onChange={(e) => set('longitude', e.target.value)}
                placeholder="เช่น 98.4863"
        ...
        <div className="flex justify-end gap-3">
          <Link
            href="/admin/parks"
            className="rounded-xl border border-forest-800/20 px-5 py-2.5 text-sm font-medium text-muted hover:bg-sand-100"
          >
            ยกเลิก
          </Link>
          <button
