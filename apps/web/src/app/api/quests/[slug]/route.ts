...
// API-QUESTS-GET (SPEC-07) — GET /api/quests/:slug
// Slug = board template slug. Same response as /api/boards/:slug but quest-framed.
export async function GET(
  ...
  try {
    ...
    if (!board) {
      return apiError('NOT_FOUND', `Quest '${slug}' not found.`, 404);
    }
    ...
    return apiError('INTERNAL_ERROR', 'Failed to fetch quest.', 500);
  }
}
